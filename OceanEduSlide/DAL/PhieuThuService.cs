...
namespace OceanEduSlide.DAL
{
    public class PhieuThuService
    {
        ...
        public void TestSyncPhieuThu(int date)
        {
            ...
            var listTHDB = _unitOfWork.PhieuThuRepository.GetQuery(a => a.NgayThanhToan != null && a.NgayThanhToan.Value.Month == day.Month && a.THDB);
            oldList.Delete();
            var phieuThuAddList = new List<BC_PhieuThu_DB>();
            var bcList = new List<ReportData>();
            var rUserWeek_RealList = new List<RevenueUser_Week_Real>();
            var listMaNV = phieuThuTakeList.Select(a => a.MaNVChotSale).Distinct().ToList();
            var listCN = phieuThuTakeList.Select(a => a.ChiNhanh).Distinct().ToList();
        ...
        public void TestSyncDthu()
        {
            var day = DateTime.Now;
            var phieuThuAllList = _dongBoTuyenSinh.BC_PhieuThu.Where(a => a.NgayThanhToan != null && a.NgayThanhToan.Value.Month == day.Month);
            var bcList = new List<ReportData>();
            var rUserWeek_RealList = new List<RevenueUser_Week_Real>();
