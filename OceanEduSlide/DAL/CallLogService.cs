...
namespace OceanEduSlide.DAL
{
    public class CallLogService
    {
        ...
        public async Task SyncYesterdayAsync2()
        {
            DateTime yesterday = DateTime.Today.AddDays(-1);
            await FetchAndSaveLogsAsync2(yesterday);
        }
        ...
        private async Task FetchAndSaveLogsAsync2(DateTime day)
        {
            string user = "lvd";
            string pass = "qazplm123`$%^";
            string baseUrl = "https://voip.ocean.edu.vn/api/report.php";
                    ...
                    newLogs = newLogs
                        .Where(log =>
                        {
                            if (userDict.TryGetValue(log.Exten, out var userId))
                            {
                                ...
                                || (a.DayEnd != null && a.DayEnd.Value >= day))).FirstOrDefault();
                                if (historyUser != null)
                                {
